...
class Solution:
    def kSmallestPairs(self, nums1, nums2, k):
        ...
        while k > 0 and pq:
            ...
            sum_val, pos = pair[0], pair[1]  # Get the smallest sum and the index of the second element in nums2

            currentPair = [sum_val - nums2[pos], nums2[pos]]
            resV.append(currentPair)  # Add the pair to the result list

            # If there are more elements in nums2, push the next pair into the priority queue
            if pos + 1 < len(nums2):
                heapq.heappush(pq, [sum_val - nums2[pos] + nums2[pos + 1], pos + 1])
